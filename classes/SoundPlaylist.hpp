...
/**
 * Defines the SoundPlaylist object, an object used to hold ordered lists of keys to Sound/Music objects in the
 * stevensSound library.
 *
 * Playlists are used for the playPlaylist() and playMusicPlaylist() functions in the stevensSound library.
 */
class SoundPlaylist
{
    public:
		...
		double trackPosition = 0.0; //Position in seconds within the current track; saved on playlist switch, restored on resume
        ...
        //Parametric
        /*
         * Given categories of sounds, and an order of tracks by name, create a vector of ordered Sound
		 ...
		 *
		 * Parameters:
         *  unordered_map<std::string, unordered_map<std::string, Sound> > & sourceSounds - The map of all sounds from the stevensSound library
        ...
        SoundPlaylist(    const std::string & nameParam,
							std::unordered_map<std::string, std::unordered_map<std::string, Sound> > & sourceSounds,
							...
							std::unordered_map<int, int> postTrackDelaysParam = {}   )
        {
			...
			for(int i = 0; i < (int)soundCategories.size(); i++)
			{
			...
			for(int i = 0; i < (int)trackOrder.size(); i++)
			{
				for(int k = 0; k < (int)soundCategories.size(); k++)
				{
